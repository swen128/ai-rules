...
// TODO: refactor
const extractLogSummaryText =
  (entry: RawLogEntry, summaryFields?: string[]) => {
...
// TODO: refactor
const processSummaryFields = R.curry(
  (fields: string[], entry: RawLogEntry): string => {
...
const extractSummaryWithoutFields = (entry: RawLogEntry) => truncate(
...
const flatMap = <T, U>(f: (arg: T) => U) => (maybe: T | undefined): U | undefined =>
  maybe === undefined ? undefined : f(maybe);

const getTextPayload = R.pipe(
  R.prop("textPayload"),
  val => typeof val === "string" ? val : undefined,
)

const getJsonMessage = R.pipe(
  R.path(["jsonPayload", "message"]),
  val => typeof val === "string" ? val : undefined,
)

const getProtoMessage = R.pipe(
  R.path(["protoPayload", "message"]),
  val => typeof val === "string" ? val : undefined,
)

const getNestedJsonMessage = R.pipe(
  R.prop("jsonPayload"),
  findMessage,
)
...
const stringifyJsonPayload = R.pipe(
  R.prop("jsonPayload"),
  flatMap(JSON.stringify),
);

const stringifyProtoPayload = R.pipe(
  R.prop("protoPayload"),
  flatMap(JSON.stringify),
);
